...
obj = getSpecialFolder(#CSIDL_PROGRAM_FILES) + "HandyHDLED\HandyHDLED.exe"
obj2 = getSpecialFolder(#CSIDL_PROGRAM_FILES) + "HandyHDLED"
lnk = getSpecialFolder(#CSIDL_ALTSTARTUP)
lnk2 = getSpecialFolder(#CSIDL_DESKTOPDIRECTORY)

; check for existence of desktop link
If FileSize(lnk2 + "HandyHDLED.lnk") = -1
  If createShellLink(obj, lnk2 + "HandyHDLED.lnk", "", "Start HandyHDLED", obj2, obj, 0) = 0
    MessageRequester("Info", "A Desktop link was created.", #PB_MessageRequester_Info)
  EndIf
EndIf

If FileSize(lnk + "HandyHDLED.lnk") = -1
; check for existence of startup link
  If createShellLink(obj, lnk + "HandyHDLED.lnk", "", "HandyHDLED startup link", obj2, obj, 0) = 0
