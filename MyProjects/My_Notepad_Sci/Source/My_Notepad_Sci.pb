...
; 	ResizeGadget(SciGadget, #PB_Ignore, #PB_Ignore, WindowWidth(#Window) - 10, WindowHeight(#Window) - MnHeight - 10)
	...
	ForEach Docum()
		ResizeGadget(Docum()\id, #PB_Ignore, #PB_Ignore, w - 3, h - PnTabHeight - MnHeight - 4)
	Next
EndProcedure
...
If OpenWindow(#Window, 0, 0, ww, hw, "My_Notepad_Sci", #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_SizeGadget | #PB_Window_MaximizeGadget | #PB_Window_MinimizeGadget)
	...
	; 	мульти
	PnTabHeight = 29 ;  предварительный оценочный размер
	PanelGadget(#Pn, 0, 0, ww, hw)
	AddDocum("")
	CloseGadgetList()
...
; 									Case #PB_EventType_LeftDoubleClick, #PB_EventType_RightDoubleClick, #PB_EventType_RightClick
									Case #PB_EventType_Change
										tmp = GetGadgetState(#Pn)
										If tmp <> -1
											*SelectElement = GetGadgetItemData(#Pn, tmp)
...
; 								ReColor()
							...
							Case mAbout
								MessageRequester("", "Автор AZJIO" + #LF$ + "14.06.2023")


							Case #mCloseDoc
								CountTab = CountGadgetItems(#Pn)
								If CountTab = 1
								...
								If Asc(Docum()\path)
									...
									If Asc(tmp$) And CheckFilename(tmp$)
										If RenameFile(Docum()\path , GetPathPart(Docum()\path) + tmp$)
											...
											tmp = GetGadgetState(#Pn)
											If tmp <> -1
												tmp2$ = GetGadgetItemText(#Pn, tmp)
												If Asc(tmp2$) = '*'
													SetGadgetItemText(#Pn, tmp, "*" + tmp$)
												Else
													SetGadgetItemText(#Pn, tmp, tmp$)
												EndIf
											EndIf
...
; 											RunProgram(GetPathPart(Docum()\path))
											RunProgram("explorer.exe", "/select," + Chr(34) + Docum()\path + Chr(34), "")
										CompilerCase #PB_OS_Linux
											RunProgram("xdg-open", Chr(34) + GetPathPart(Docum()\path) + Chr(34), "")
									CompilerEndSelect
								EndIf
							Case #mpPath
...
; 			flgPath = 1
			If CountTab
				OpenGadgetList(#Pn, #Pn)
			EndIf
			AddGadgetItem(#Pn, CountTab, GetFilePart(Path$))
			Docum()\path = Path$
...
; 				flgFile0 = 0
; 				CountTab = 0
; 				SetGadgetItemText(#Pn, CountTab, GetFilePart(Path$))
; 			Else
; 				AddGadgetItem(#Pn, CountTab, GetFilePart(Path$))
			EndIf
		Else
			If CountTab
				OpenGadgetList(#Pn, #Pn)
			EndIf
			AddGadgetItem(#Pn, CountTab, "New" + Str(CountTab))
			Docum()\fs = #PB_UTF8
		EndIf
; 		If flgFile0
		ww = WindowWidth(#Window)
		hw = WindowHeight(#Window)
		Docum()\id = ScintillaGadget(#PB_Any, 0, 0, ww - 3, hw - PnTabHeight - MnHeight - 4, @SciNotification())
...
; 		ind = GetGadgetState(#Pn)
		SetGadgetState(#Pn, CountTab)
		SetGadgetItemData(#Pn, CountTab, *Element)
		SciGadget = Docum()\id
		SetActiveGadget(SciGadget)
...
Procedure DropFiles(StringFiles$)
	...
	ForEach Files()
		...
		For i = 0 To CountTab - 1
			*SelectElement = GetGadgetItemData(#Pn, i)
...
; 				If \modificationType & #SC_MOD_DELETETEXT
			...
			Case #SCN_SAVEPOINTREACHED ; сохранено
				...
				If tmp <> -1 And Asc(tmp$) = '*'
					If Asc(Docum()\path)
						SetGadgetItemText(#Pn , tmp , GetFilePart(Docum()\path))
					Else
						SetGadgetItemText(#Pn , tmp , Mid(tmp$, 2))
					EndIf
; 					Debug "REACHED"
				EndIf
			Case #SCN_SAVEPOINTLEFT	   ; требует сохраненения
				...
				If tmp <> -1
					If  Asc(Docum()\path)
						...
						; 					Debug "LEFT"
					Else
						SetGadgetItemText(#Pn , tmp , "*" + GetGadgetItemText(#Pn , tmp))
					EndIf
				EndIf
...
Procedure StartFile()
	If Asc(Docum()\path)
		CompilerSelect #PB_Compiler_OS
			...
			CompilerCase #PB_OS_Linux
; 				RunProgram("xdg-open", Chr(34) + Docum()\path + Chr(34), "")
				RunProgram("xdg-open", Chr(34) + Docum()\path + Chr(34), Chr(34) + GetPathPart(Docum()\path) + Chr(34))
		CompilerEndSelect
	EndIf
EndProcedure
...
; FirstLine = 1675
; Folding = ------------
; Markers = 911,1348,2440,2821
; EnableAsm
; EnableXP
; DPIAware
; UseIcon = icon.ico
; Executable = My_Notepad_Sci.exe
; CompileSourceDirectory
; DisableCompileCount = 4
; EnableBuildCount = 1
; EnableExeConstant
; IncludeVersionInfo
; VersionField0 = 0.3.5.%BUILDCOUNT
; VersionField2 = AZJIO
