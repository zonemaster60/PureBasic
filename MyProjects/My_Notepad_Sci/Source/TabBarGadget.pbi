...
;  modified by  AZJIO. Added #TabBarGadget_EventType_RightDown
...
;|                     (Martin Guttmann alias STARGЕTE)
;|  PureBasic        : 5.20+
...
; Attribute fьr das TabBarGadget
Enumeration
	...
	#TabBarGadget_NormalTabLength      = 1<<26 ; fьr Spдter
...
Enumeration #PB_EventType_FirstCustomValue
	...
	#TabBarGadget_EventType_Change       ; Der aktive Tab wurde geдndert
	#TabBarGadget_EventType_Resize       ; Die grцЯe der Leiste hat sich geдndert
	...
	#TabBarGadget_EventType_EditItem     ; der Text einer Karte wurde geдndert
	#TabBarGadget_EventType_CheckBox     ; der Status der Checkbox hat sich geдndert
	#TabBarGadget_EventType_PopupButton	 ; der Popup-Button wurde gedrьckt
	#TabBarGadget_EventType_RightDown
EndEnumeration




; Positionskonstanten fьr "Item"-Befehle
Enumeration
...
; Sortierter Eintrag fьr die Textkьrzung
...
Structure TabBarGadgetRow
	Length.i  ; Aktuelle Lдnge einer Zeile
	Items.i   ; Aktuelle Anzahl der Tabs
EndStructure

; Farben fьr einen Eintrag
Structure TabBarGadgetItemColor
	Text.i        ; Textfarbe
	Background.i  ; Hintergrundfarbe
EndStructure

; Lage und GrцЯe einer Registerkarte 
Structure TabBarGadgetItemLayout
	X.i         ; X-Position
	Y.i         ; Y-Position
	Width.i     ; (innere) Breite
	Height.i    ; (innere) Hцhe
	PaddingX.i  ; VergrцЯerung (z.B. bei aktiver Registerkarte)
	PaddingY.i  ; 
	CrossX.i    ; Position des SchlieЯen-X
	CrossY.i    ;
...
Structure TabBarGadgetItem
	...
	Selected.i                              ; Aktuell ausgewдhlt
	Checked.i                               ; Abgehakt
	ToolTip.s                               ; ToolTop
	Length.i                                ; Lдnge in Pixel (TEMP)
	Row.i                                   ; Zeile (TEMP)
	...
	*PreviousSelectedItem.TabBarGadgetItem  ; Zuvor ausgewдhlter Tab
EndStructure
...
Structure TabBarGadgetToolTip
	...
	*Old                             ; Alte ToolTip-Adresse
	ItemText.s                       ; Text fьr die Registerkarte
	NewText.s                        ; Text fьr die "Neu"-Registerkarte
	CloseText.s                      ; Text fьr den SchlieЯen-Button
EndStructure
...
Structure TabBarGadgetEditor
	*Item.TabBarGadgetItem  ; Zu Bearbeitende Karte
	...
	Selection.i             ; Textmarkierungslдnge
	Selectable.i            ; Ob die Mausbewegung zu einer Textmarkierung fьhrt
EndStructure

; Layout der Leiste
Structure TabBarGadgetLayout
	PreviousButtonX.i  ; Position des "zurьck" Navigationspfeil
	PreviousButtonY.i
	NextButtonX.i  ; Position des "vor" Navigationspfeil
	NextButtonY.i
	PopupButtonX.i ; Position des Popup-Pfeils
	PopupButtonY.i
	ButtonWidth.i  ; GrцЯe der Buttons
	ButtonHeight.i
	ButtonSize.i
EndStructure
...
Structure TabBarGadget
	...
	*LockedItem.TabBarGadgetItem      ; Registerkarte angeschlagen wurde (fьr Klicks)
	LockedClose.i                     ; SchlieЯenbutton angeschlagen
	LockedCheck.i                     ; SchlieЯenbutton angeschlagen
	...
	FocusingSelectedTab.i             ; muss die ausgewдhlte Registerkarte fokussiert werden
	MaxLength.i                       ; maximal nutzbare Lдnge fьr Karten
	Length.i                          ; Lдnge aller sichtbaren Karten
	Radius.i                          ; Radius der Kartenrundung
	MinTabLength.i                    ; minimale Lдnge einer Karte
	MaxTabLength.i                    ; maximale Lдnge einer Karte
	NormalTabLength.i                 ; normale Lдnge einer Karte
	TabTextAlignment.i                ; Textausrichtung
	ToolTip.TabBarGadgetToolTip       ; ToolTip
	TabSize.i                         ; GrцЯer einer Registerkarte
	Rows.i                            ; Anzahl der Zeilen
	Resized.i                         ; Das Gadget muss vergrцЯert werden
	Editor.TabBarGadgetEditor         ; Editor fьr eine Karte
	Layout.TabBarGadgetLayout         ; Layout der Leiste
	UpdatePosted.i                    ; Nach einem PostEvent #True
EndStructure

; Timer fьr das kontinuierliche Scrollen
...
; Include fьr das Registerkartenleisten-Gadget
Structure TabBarGadgetInclude
	TabBarColor.i                   ; Hintergrundfarbe des Gadgets
	FaceColor.i                     ; Hintergrundfarbe einer Karte
	HoverColorPlus.i                ; Farbдnderung fьr den Hover-Effekt
	ActivColorPlus.i                ; Farbдnderung fьr aktuell ausgewдhlte Karten
	...
	Margin.i                        ; AuЯenabstand (Rahmen zu Gadget-Rand)
	ImageSpace.i                    ; Freiraum zwischen Bild und Text
	CloseButtonSize.i               ; GrцЯe des SchlieЯenkreuzes
	CheckBoxSize.i
	ImageSize.i
	ArrowSize.i                     ; GrцЯe des Navigationspfeils
	ArrowWidth.i                    ; 
	ArrowHeight.i                   ;
	Radius.i                        ; Radius der Abrundung der Karte
	MinTabLength.i                  ; Mimimale Lдnge einer Karte
	MaxTabLength.i                  ; Maximale Lдnge einer Karte
	TabTextAlignment.i
	VerticalTextBugFix.f            
	NormalTabLength.i               ; [fьr spдter]
	FadeOut.i                       ; Lдnge der Farbausblendung bei einer Navigation
	...
	Timer.TabBarGadget_Timer        ; Timer fьr das kontinuierliche Scrollen
EndStructure
...
; Diese Werte kцnnen sowohl im Include, als auch im Hauptcode spдter ьber TabBarGadgetInclude\Feld geдndert werden.
With TabBarGadgetInclude
	CompilerSelect #PB_Compiler_OS
		...
		CompilerDefault
			\TabBarColor   = $FF3F3F3F ; Фон без вкладок
			\BorderColor   = $FF666666 ; граница
			\FaceColor     = $FF444444 ; цвет вкладок (градиент этими цветами не задаётся)
			\TextColor     = $FFCCCCCC ; цвет текста
	CompilerEndSelect
	\HoverColorPlus               = $FF002000 ; $FF371717
	\ActivColorPlus               = $FF202000
...
;ЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇЇ



; Gitb die Adresse (ID) der Registerkarte zurьck.
...
; Wдhlt die angegebene Karte aus und aktualisiert die Select-Hierarchie
...
; Wдhlt die angegebene Karte ab und aktualisiert die Select-Hierarchie
Procedure TabBarGadget_UnselectItem(*TabBarGadget.TabBarGadget, *Item.TabBarGadgetItem) ; Code OK
	...
	*Item\Selected = #False
	ForEach *TabBarGadget\Item()
		...
		EndIf
	Next
	If *TabBarGadget\SelectedItem = *Item ; Auswahl muss geдndert werden
...
; Gibt #True zurьck, wenn die Maus innerhalb des Rechtecks ist.
;   Width und Height kцnnen auch negativ sein.
...
; Gibt die Lдnge der Registerkate zurьck.
Procedure.i TabBarGadget_ItemLength(*TabBarGadget.TabBarGadget, *Item.TabBarGadgetItem) ; Code OK
	...
	If *TabBarGadget\Attributes & #TabBarGadget_Vertical
		TextLength * TabBarGadgetInclude\VerticalTextBugFix ; 5% lдnger, wegen Ungenaugikeit von TextWidth bei Rotation
...
; Gibt den maximal zur verfьgungstehenden Platz fьr Registerkarten zurьck.
...
; Fьhrt eine Textkьrzung durch, bis alle Karte in die Leiste passen.
Procedure.i TabBarGadget_TextCutting(*TabBarGadget.TabBarGadget) ; Code OK
	...
	With *TabBarGadget
		...
		; Der Textlдnge nach (groЯ -> klein) sortierte Eintrдge anlegen.
		ForEach \Item()
		...
		; Textkьrzung durchfьhren, bis alle Karte in die maximale Breite passen.
...
; Rotiert das Image abhдngig von der Leistenausrichtung 
...
; (Er-)setz ein neues Icon fьr die Karte
...
Procedure TabBarGadget_Examine(*TabBarGadget.TabBarGadget)
	...
	With *TabBarGadget
		...
		If \HoverItem
			...
			; Tooltip aktualisieren & SchlieЯenbutton & Checkbox
...
Procedure TabBarGadget_Update(*TabBarGadget.TabBarGadget)
	...
	With *TabBarGadget
		...
		Else
			...
			; ggf. aktuell ausgewдhlte Registerkarte in den sichtbaren Bereich bringen
...
; Dauerschleife fьr das automatische Scrollen in der Navigation
...
; Fьhrt eine Aktualisierung (Neuzeichnung) des Gadgets durch.
...
; Fьgt eine Registerkarte an die angegebenen Position ein.
...
; Gibt die einmalige ID der angegebenen Registerkarte zurьck.
...
; Gibt die Anzahl der Registerkarten zurьck.
...
; Setz einen ToolTip fьr die Registerkartenleiste (fьr die Registerkarten, die "Neu"-Registerkarte und den SchlieЯenbutton)
...
; Setz einen ToolTip fьr die Registerkarte.
...
; Дndert den Wert eines Attributs der Registerkartenleiste.
...
; Gibt den Wert eines Attributs der Registerkartenleiste zurьck.
...
; Дndert den Daten-Wert der Registerkartenleiste.
...
; Gibt den Daten-Wert der Registerkartenleiste zurьck.
...
; Дndert die zu nutzende Schrift.
...
; Дndert den Status der Registerkartenleiste.
...
; Gibt den Status der Registerkartenleiste zurьck.
...
; Gibt den Text der aktuell ausgewдhlten Registerkarte zurьck.
...
; Дndert die Attribute der angegebenen Registerkarte.
...
; Gibt den Status der angegebenen Registerkarte zurьck.
...
; Дndert den Datenwert der angegebenen Registerkarte.
...
; Gibt den Datenwert der angegebenen Registerkarte zurьck.
...
; Дndert die Farbe der angegebenen Registerkarte.
...
; Gibt die Farbe der angegebenen Registerkarte zurьck.
...
; Дndert das Icon der angegebenen Registerkarte.
...
; Дndert die Position der angegebenen Registerkarte (die Registerkarte wird also verschoben).
...
; Gibt die Position der angegebenen Registerkarte zurьck.
...
; Дndert den Status der angegebenen Registerkarte.
...
; Дndert den Text der angegebenen Registerkarte.
...
; Gibt den Text der angegebenen Registerkarte zurьck.
