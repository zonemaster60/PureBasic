...
Procedure mnuOpen(eventid)
  ...
  If sOpenfile <> ""
    ...
    If hFile = 0
      MessageRequester("Error", "Couldn't open file!",#PB_MessageRequester_Error | #PB_MessageRequester_Ok)
    Else
      Protected sBuf.s
...
Procedure SaveToFile(sFile.s)
    ...
    If hFile = 0
      MessageRequester("Error", "Couldn't create file!",#PB_MessageRequester_Error | #PB_MessageRequester_Ok)
    Else
...
Procedure mnuAbout(eventid)
  Define version.s = "0.0.0.1"
  MessageRequester("Info","HandyTXTPad v"+version+#CRLF$+
                          "A handy little text editor",#PB_MessageRequester_Info | #PB_MessageRequester_Ok)
EndProcedure
