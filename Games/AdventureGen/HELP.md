...
- Wizard mode uses the actual values loaded from `src\data\themes.csv`, including goals and twists.
- Some rooms may be dark, and the starter torch can be used to reveal details in those spaces.
- Rooms can contain takeable items, healing supplies, and hostile presences that can be examined or fought.
- Defeating the guardian reveals a sigil; bring it back to the landmark and **USE** it there to win the adventure.

### 6. Technical Details
- **Primary source file**: `src\main.pb`
- **Embedded help file**: `src\HELP.md`
- **Theme data**: `src\data\themes.csv`
- **Support modules**: `src\app_core.pbi`, `src\app_helpers.pbi`, `src\app_content.pbi`, `src\app_world.pbi`, `src\app_export.pbi`, `src\app_ui.pbi`
- **Requirements**: PureBasic v6.30 (x64) and `pbcompiler` in your PATH if you want to use **BUILD**

### 7. Starting the Adventure
1. Run `AdventureGen.exe` or open `src\main.pb` in PureBasic.
