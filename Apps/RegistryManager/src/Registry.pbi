...
; Date:            Sep 04, 2015
; Version:         1.5.0 (Updated for PB 6.30 compatibility)
; Target Compiler: PureBasic 5.2+ / 6.30+
; Target OS:       Windows
...
Module Registry
  ...
  Procedure.s ReadValue(topKey, KeyName.s, ValueName.s = "", WOW64 = #False, *Ret.RegValue = 0)
    ...
    If lpcbData
      ; Allocate an extra Character so PeekS() always has a terminator
      ; even if the registry data isn't null-terminated.
      *lpData = AllocateMemory(lpcbData + SizeOf(Character))
      If *lpData = 0
        If *Ret <> 0
        ...
        ProcedureReturn ""
      EndIf
      FillMemory(*lpData, lpcbData + SizeOf(Character), 0)
    Else
      ; Some values exist but have 0 length; return "" with proper type.
      *lpData = AllocateMemory(SizeOf(Character))
      If *lpData
        PokeS(*lpData, "", 1)
      EndIf
    EndIf
    ...
    Select lpType
      Case #REG_BINARY
        ...
        If *Ret <> 0
          *Ret\BINARY = *lpData
          *Ret\SIZE = lpcbData
          ProcedureReturn result ; We don't free the memory when returning BINARY as a string, because the pointer is stored in *Ret
        EndIf
        ; If *Ret is 0, we must free the memory
        FreeMemory(*lpData)
        ProcedureReturn result
      ...
      Case #REG_DWORD
       ...
       Case #REG_MULTI_SZ
         ; MULTI_SZ is a sequence of NUL-terminated strings terminated by an extra NUL.
         ; Iterate string-by-string instead of character-by-character.
         While i < lpcbData
           Protected part.s
           part = PeekS(*lpData + i)
           If part = ""
      ...
      Case #REG_QWORD
       ...
       Case #REG_SZ
         ; Ensure we don't read beyond the returned buffer.
         result = PeekS(*lpData, lpcbData / SizeOf(Character))
         If *Ret <> 0
           *Ret\STRING = result
           *Ret\SIZE = Len(result)
         EndIf
...
; EnableAdmin
; DPIAware
; Executable = HandyHDLED\HandyHDLED.exe
; DisableDebugger
